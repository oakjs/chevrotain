...
export class Parser {
    ...
    protected implicitRuleInvocationStateUpdate(fullName:string):void {
        this.RULE_OCCURRENCE_STACK.push(1)
        this.RULE_STACK.push(fullName)

        // NOOP when cst is disabled
        this.updateCstStack(fullName)
    }

    protected implicitRuleFinallyStateUpdate():void {
        this.RULE_STACK.pop()
        this.RULE_OCCURRENCE_STACK.pop()
    ...
    private saveRecogState():IParserState {
        ...
        // TODO: add extra explict stack here
        return {
            errors:     savedErrors,
    ...
    private optionInternal<OUT>(actionORMethodDef:GrammarAction<OUT> | DSLMethodOpts<OUT>, occurrence:number):OUT {
        ...
        let isExplictRule = (<DSLMethodOpts<OUT>>actionORMethodDef).NAME !== undefined
        // TODO: measure performance impact of try/catch, consider
        try {
            if (isExplictRule) {
                // TODO: but this uses the rulestack directly
                let key = this.getKeyForAutomaticLookahead(OPTION_IDX, occurrence)
                // TODO: build full name here
                // TODO: how to differentiate between real and implicit rules? - or does it not matter?
                // TODO: what is faster? cache it with the same key used in "getLookaheadFuncForOption" ? or concat strings?
                this.implicitRuleInvocationStateUpdate((<DSLMethodOpts<OUT>>actionORMethodDef).NAME)
            }
            ...
            if (isExplictRule === true) {
                this.implicitRuleFinallyStateUpdate()
            }
        }
    }
